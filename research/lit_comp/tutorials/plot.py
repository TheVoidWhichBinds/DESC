...
def plot_iota_profiles(
    ...
    """
    Plot iota profiles for all optimization groups.
    """
    ...
    for optimization_name, group_files in files.items():
        ...
        try:
            profiles = load_profiles_for_group(
            ...
            print("")
            print(f"Skipping iota plot for {output_stem}: {error}")
            print("")
            continue

        save_path = tutorial_dir / f"{output_stem}_iota.png"

        title = f"{output_stem}: iota profile"
...
def plot_pressure_gradient_profiles(
        tutorial_name,
        tutorial_dir,
        files,
    ):
    """
    Plot dp/drho profiles for all optimization groups.
    """
    ...
    for optimization_name, group_files in files.items():
        ...
        profiles = load_pressure_gradient_profiles_for_group(
            group_files = group_files,
        )

        save_path = tutorial_dir / f"{output_stem}_dp_drho.png"

        title = f"{output_stem}: pressure gradient"
...
def plot_balloon(
    ...
    saved_paths += plot_pressure_gradient_profiles(
        tutorial_name = tutorial_name,
        tutorial_dir = tutorial_dir,
        files = files,
    )
